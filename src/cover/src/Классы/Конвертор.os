...
Процедура ПодготовитьЛог(ИсходныйТекст) Экспорт
	...
	ПолеДанныеПокрытия.РазобратьЛог(Лог, ДанныеКонфигурации, Неопределено);
...
Процедура СформироватьПустойКовер(ЗаписьXML, ИмяФайла, ПутиКИсходникам) Экспорт
	...
	Пока НЕ Строка = Неопределено Цикл 
		...
		Если НЕ ПустаяСтрока(СокрЛПСтрока) 
			...
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "Function")
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "Procedure")
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "&") 
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "#") 	
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "Перем ")
			И НЕ СтрНачинаетсяС(СокрЛПСтрока, "Var ") Тогда
