...
Процедура РазобратьЛог(Лог, ДанныеКонфигурации, ДанныеОбработок) Экспорт
	...
	Для каждого Ключ Из Данные Цикл
		...
		Иначе	
			Если НЕ Ключ.moduleId.Свойство("extID") Тогда
				Ключ.moduleId.Вставить("extID", -1);
			КонецЕсли;
			Модуль = ДанныеКонфигурации.ОпределитьМодульПоId(Ключ.moduleId.objectID, Ключ.moduleId.propertyID, Ключ.moduleId.extID);
		КонецЕсли;
