...
Функция ПутьКонфигуратора(Модуль) Экспорт
	...
	КонецЕсли;
	Если ЧастиИмен.ВГраница() > 2 Тогда
		...
		ИначеЕсли ЧастиИмен[ЧастиИмен.ВГраница() - 2] = "Command" Тогда
			ПутьМетаданного = ПутьМетаданного + Разделитель + "Commands" + Разделитель + ЧастиИмен[ЧастиИмен.ВГраница() - 1];			
		КонецЕсли;	
	КонецЕсли;

	ПутьМетаданного = ПутьМетаданного + Разделитель + "Ext";
	Если ЧастиИмен[0] = "CommonCommand" Тогда
		Путь = ПутьМетаданного + Разделитель + "CommandModule.bsl";
	Иначе
